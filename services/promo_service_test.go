...
func TestPromoEligibility(t *testing.T) {
	...
	cases := []struct {
		...
		{"quota exhausted", func(p *promo_model.Promo) { limit := 5; p.UsageLimit = &limit; p.UsageCount = 5 }, 100000, "Batas kuota voucher habis"},
		{"zero quota exhausted", func(p *promo_model.Promo) { limit := 0; p.UsageLimit = &limit }, 100000, "Batas kuota voucher habis"},
		{"quota remaining", func(p *promo_model.Promo) { limit := 5; p.UsageLimit = &limit; p.UsageCount = 4 }, 100000, ""},
